...
def table_to_markdown(table):
    ...
    for row in table:
        ...
        for child in row.children:
            ...
            if child.type == "video":
                content += "{{{{< youtube id=\"{0}\" >}}}}<br>".format(
                    child.source[-11:])
            elif child.type == "text":
                content += "{0}<br>\n".format(child.title)
            elif child.type == "header":
                content += "# {0}<br>".format(child.title.strip())
            elif child.type == "embed":
                content += "{0}<br>".format(child.source)
            elif child.type == "bookmark":
                bookmark_cover = child.bookmark_cover
                content += "<br>{{{{< customfig target=\"{link}\" class=\"notion-bookmark\" caption=\"{caption}\" title=\"{title}\" link=\"{link}\" src=\"{image}\" >}}}}".format(
                    image=bookmark_cover, link=child.link, title=child.title, caption=child.description)
        ...
        yield (row.title, row.createdat, content, row.published, link, row.tags)
...
if __name__ == "__main__":
    ...
    for row in table:
        ...
        with open("./bitwig-resources/content/posts/{name}.md".format(name=title), "w") as f:
            f.write(HUGO_TEMPLATE.format(title=title,
                                         date=date, content=content, link=url, tags=tags))
